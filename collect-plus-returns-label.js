function CollectPlusReturnsLabel(container, guid) {
    "use strict";
    var _container = container;
    var _guid = guid;
    var debug = true;
        
    var entityMap = {
        "&": "&amp;",
        "<": "&lt;",
        ">": "&gt;",
        '"': '&quot;',
        "'": '&#39;',
        "/": '&#x2F;'
    };

    function escapeHtml(string) {
        return string ? String(string).replace(/[&<>"'\/]/g, function (s) {
            return entityMap[s];
        }) : '';
    }

    function pad(num, size) {
        var s = "0000000" + num;
        return s.substr(s.length - size);
    }

    function mattr(name) {
        return _container.getAttribute(name) || console.error('collect+ attribute missing [' + name + ']') || name.substr(5);
    }

    function oattr(name, def) {
        return _container.getAttribute(name) || def || '';
    }

    function today() {
        var date= new Date();
        return pad(date.getDate(), 2) + '/' + pad(date.getMonth() + 1, 2) + '/' + date.getFullYear();
    }

    function make() {
        var barcode = '8' + mattr('data-tac') + pad(mattr('data-sequence'), 8) + 'A0' + mattr('data-depot-number');

        var label = debug ?
            '<link href="collect-plus-returns-label.css" rel="stylesheet" />' :
            '<style>@page{size:A5;margin-top:2mm;}.cp_table{margin-top:10px;font-family:Tahoma,sans-serif;font-size:22px;font-weight:700;border-collapse:collapse;width:144mm}.cp_table td{border:2px solid #000;vertical-align:top;padding:8px}.cp_details__container{padding:0!important}.cp_header{line-height:75px;font-size: 1.3em}.cp_header img{float:right}.cp_cell{padding:8px;border-top:2px solid #000}.cp_cell:first-of-type{border-top-width:0}.cp_table label{font-weight:400;font-size:60%;display:block}.cp_details__container{width:40%;display:table-cell}.cp_address{font-weight:400;padding-left:15px!important;padding-right:15px!important}.cp_center{text-align:center}.cp_postcode{font-weight:700;font-size:1.3em}.cp_barcode{width:70mm;height:35mm}</style>';
        label += '<table class="cp_table"><tr><td class="cp_header" colspan="2"><img width="189" height="75" title="" alt="" src="data:image/jpeg;base64," />';
        label += escapeHtml(mattr('data-client-name'));
        label += '</td></tr><tr><td class="cp_details__container"><div class="cp_cell"><label>Depot</label>';
        label += escapeHtml(mattr('data-depot-number') + ' ' + mattr('data-depot-short'));
        label += '</div><div class="cp_cell"><label>Service</label>';
        label += escapeHtml(mattr('data-service'));
        label += '</div><div class="cp_cell"><label>Label Date</label>';
        label += escapeHtml(oattr('data-label-date', today()));
        label += '</div><div class="cp_cell"><label>Returns Reference</label>';
        label += escapeHtml(oattr('data-returns-reference', 'N/A'));
        label += '</div></td><td class="cp_address"><label>&nbsp;</label>';
        label += '<div>' + escapeHtml(mattr('data-center-name')) + '</div>';
        label += '<div>' + escapeHtml(mattr('data-address-1')) + '</div>';
        label += '<div>' + escapeHtml(oattr('data-address-2')) + '</div>';
        label += '<div>' + escapeHtml(oattr('data-address-3')) + '</div>';
        label += '<div>' + escapeHtml(oattr('data-address-4')) + '</div>';
        label += '<div class="cp_postcode">';
        label += escapeHtml(mattr('data-address-postcode'));
        label += '</div></td></tr><tr class="cp_center"><td colspan="2"><label>Storekeeper Instruction</label>GIVE TO THE DRIVER</td></tr><tr class="cp_center"><td colspan="2"><div>';
        label += '<img class="cp_barcode" id="' + _guid + '"/>';
        label += '</div>';
        label += escapeHtml(barcode).replace(/0/g, '&Oslash;');
        label += '</td></tr></table>';

        var ifrm = document.createElement("IFRAME");
        ifrm.style.display = debug ? 'inline' : 'none';
        _container.appendChild(ifrm);

        ifrm = (ifrm.contentWindow) ? ifrm.contentWindow : (ifrm.contentDocument.document) ? ifrm.contentDocument.document : ifrm.contentDocument;
        ifrm.document.open();
        ifrm.document.write(label);
        ifrm.document.close();

        JsBarcode(ifrm.document.getElementById(_guid), barcode, {width: 2, height: 125});
        _container.addEventListener('click', function() {
            ifrm.document.execCommand('print', false, null);
            return false;
        });
    }
    make();
}

document.addEventListener("DOMContentLoaded", function() {
    var labels = document.getElementsByClassName('cp_returns_label');
    for(var i= 0; i < labels.length; ++i) {
        CollectPlusReturnsLabel(labels[i], 'label-id-' + i)
    }
});
